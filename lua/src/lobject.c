...
static char chLocaleDecimalChar = '.';
static char bCharSet = 0;
...
//R.K. part of my locale fixes, some times the decimal point
//is different for some locales it is a comma
//so check if it needs changing and do it
static char *changeForLocale(char *s){
	
	char *szTemp=s;
	if(!bCharSet){
		bCharSet=1;
		chLocaleDecimalChar=getDecimalChar();
		printf("[code] current decimal char: %c\n",chLocaleDecimalChar);
	}
	
	if(chLocaleDecimalChar!='.')
	{
		while(*szTemp){
			if(*szTemp=='.'){
				*szTemp=chLocaleDecimalChar;
				break;
			}
			szTemp++;
		}
...
int luaO_str2d (char *s, Number *result) {  /* LUA_NUMBER */
  char *endptr;
  Number res;
  s=changeForLocale(s);
